...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    async def send(self, chat_id: str, content: str, reply_to: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> SendResult:
        ...
        # Normalize markdown for DingTalk; the card title shows the message head, not a fixed "Hermes"
        title = self._normalize_markdown(content[: 64])
        payload = {"msgtype": "markdown", "markdown": {"title": title, "text": self._normalize_markdown(content[: self.MAX_MESSAGE_LENGTH])}}
        try:
